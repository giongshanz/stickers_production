...
- Continue the existing plan: 50 topics, 8 slots each, 390 unique asset IDs, 10 shared IDs used in exactly two topics. Do not restart or regenerate existing masters by default.
...
6. End each generation batch with current requests/logs, QA, `refresh`, and `verify`. State the real unique count, remaining count and material QA limitations. Do not call the 50-topic objective complete while images or required QA remain.
